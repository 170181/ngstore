angular.module("storeapp").controller("ClienteEditController", ['$scope', '$log', '$location', '$routeParams', 'ClienteService', function($scope, $log, $location, $routeParams, ClienteService) {
    ...
    if($routeParams.id != null) {
        ClienteService.buscarPorId($routeParams.id).then(function(responseObj) {
            $scope.cliente = responseObj;
        })
        .catch(function(error) {
            alert("Não foi possível abrir o cliente com id " + $routeParams.id + " para edição. Motivo: " + error.statusText);
            $log.log(error);
